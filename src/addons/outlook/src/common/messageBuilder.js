...
function buildPolycomToken(sipNumber, isWeb, domain) {
  ...
  if (isWeb) {
    return `<span style="display:none; font-size:1px; color:#ffffff; line-height:1px; max-height:0px; max-width:0px; opacity:0; overflow:hidden; mso-hide:all;">${polycomRaw.replace(/\n/g, "<br>")}</span>`;
  }
  return polycomRaw;
}
...
function buildMeetingMessage(data, isWeb) {
  ...
  const sipFromPin = data.telephony?.pin_code
    ? String(data.telephony.pin_code).replace(/\s+/g, "")
    : "";
